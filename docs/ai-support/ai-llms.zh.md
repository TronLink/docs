...
| 端点 | 说明 |
| --- | --- |
| [/docs/llms.txt](/docs/llms.txt) | 英文版精选索引,每条带一句话描述（[llmstxt.org](https://llmstxt.org/) 规范） |
| [/docs/llms.zh.txt](/docs/llms.zh.txt) | 中文版精选索引——同样的版式，链接指向 `/docs/zh/` 下的中文页面 |
| [/docs/llms-full.en.txt](/docs/llms-full.en.txt) | 所有英文页面全文聚合，便于单次抓取 |
| [/docs/llms-full.zh.txt](/docs/llms-full.zh.txt) | 所有中文页面全文聚合，便于单次抓取 |
| [/docs/llms-full.txt](/docs/llms-full.txt) | `llms-full.en.txt` 的别名（保留以兼容旧链接） |

生产地址：`https://docs.tronlink.org/docs/llms.txt`、`https://docs.tronlink.org/docs/llms.zh.txt`，以及对应的 `llms-full.*.txt` 全文聚合。
...
| 导航 / 找到正确的中文页面 | `llms.zh.txt` —— 中文描述、同样的版式 |
| 单次抓取整份文档 | `llms-full.en.txt` / `llms-full.zh.txt` —— 按语言区分的全文 |

请先用对应语言的索引并跟随其链接；需要一次性获取全部内容时再抓 `llms-full.*` 全文包。中文索引指向 `/docs/zh/` 下的页面，英文索引指向 `/docs/` 下的页面。
